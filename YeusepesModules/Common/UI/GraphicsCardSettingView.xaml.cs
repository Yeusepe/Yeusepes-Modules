...
namespace YeusepesModules.Common.ScreenUtilities
{
    public partial class GraphicsCardSettingView : UserControl
    {
        ...
        public GraphicsCardSettingView(VRCOSC.App.SDK.Modules.Module module, ModuleSetting setting)
        {
            InitializeComponent();
            ...
            var screenUtilitiesField = module.GetType().GetField("screenUtilities", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
            if (screenUtilitiesField != null)
            {
                ScreenUtilities screenUtilities = (ScreenUtilities)screenUtilitiesField.GetValue(module);
                availableGPUs = screenUtilities.GetGraphicsCards();
            }
        }
